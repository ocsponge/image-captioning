...
if __name__=='__main__':
    ...
    images_names = torch.load(IMAGES_FEATURE_PATH)['images_names']
    images_names = images_names[:int(len(images_names)/5)]
    images_captions_0 = images_captions_0.set_index('image_id').reindex(index=images_names).reset_index()
    images_captions_1 = images_captions_1.set_index('image_id').reindex(index=images_names).reset_index()
    images_captions_2 = images_captions_2.set_index('image_id').reindex(index=images_names).reset_index()
    images_captions_3 = images_captions_3.set_index('image_id').reindex(index=images_names).reset_index()
    images_captions_4 = images_captions_4.set_index('image_id').reindex(index=images_names).reset_index()
    ...
    if RUN_MODE == 'train':
        ...
        vocabulary = {v:(k+3) for k,v in enumerate(vocabulary)}
        vocabulary.update({'<start>':0, '<end>':1, '<unknown>':2})
    ...
    images_captions_all.caption = images_captions_all.caption.map(lambda line: [0]+[vocabulary[word] for word in line]+[1])
